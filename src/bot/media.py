import io
import logging
import os
import tempfile
from typing import Any, Dict, Optional, Tuple

from telegram import Voice
from telegram.ext import ContextTypes

from src.message_store import get_message_by_telegram_message_id
from src.model_provider import ModelProvider

NON_TEXT_REPLY_PLACEHOLDER = "[non-text message]"
IMAGE_MAX_BYTES = 15 * 1024 * 1024
_MIME_BY_SUFFIX = {
    ".jpg": "image/jpeg",
    ".jpeg": "image/jpeg",
    ".png": "image/png",
    ".webp": "image/webp",
    ".gif": "image/gif",
    ".bmp": "image/bmp",
    ".tif": "image/tiff",
    ".tiff": "image/tiff",
}


def transcribe_audio(audio_path: str, model_provider: ModelProvider) -> str:
    return model_provider.transcribe(audio_path)


def guess_mime_from_name(name: str) -> str:
    lowered = str(name or "").lower()
    for suffix, mime_type in _MIME_BY_SUFFIX.items():
        if lowered.endswith(suffix):
            return mime_type
    return ""


def is_image_document(document: Any) -> Tuple[bool, str]:
    mime = (getattr(document, "mime_type", "") or "").lower()
    filename = (getattr(document, "file_name", "") or "").lower()
    guessed_mime = guess_mime_from_name(filename)
    is_image = mime.startswith("image/") or bool(guessed_mime)
    effective_mime = mime if mime.startswith("image/") else guessed_mime
    return is_image, effective_mime


def combine_caption_and_extracted(caption: str, extracted: str) -> str:
    caption_clean = (caption or "").strip()
    extracted_clean = (extracted or "").strip()
    if caption_clean and extracted_clean:
        return f"{caption_clean}\n{extracted_clean}".strip()
    return (caption_clean or extracted_clean).strip()


async def extract_text_from_photo_message(
    message: Any,
    context: ContextTypes.DEFAULT_TYPE,
    *,
    model_provider: ModelProvider,
) -> str:
    if not getattr(message, "photo", None):
        return ""
    photo = message.photo[-1]
    file = await context.bot.get_file(photo.file_id)
    bio = io.BytesIO()
    await file.download_to_memory(bio)
    image_bytes = bio.getvalue()
    extracted = model_provider.image_to_text(image_bytes, mime_type="image/jpeg")
    return combine_caption_and_extracted(
        getattr(message, "caption", "") or "", extracted
    )


async def extract_text_from_image_document(
    message: Any,
    context: ContextTypes.DEFAULT_TYPE,
    *,
    model_provider: ModelProvider,
) -> Optional[str]:
    document = getattr(message, "document", None)
    if document is None:
        return None
    is_image, effective_mime = is_image_document(document)
    if not is_image:
        return None
    file = await context.bot.get_file(document.file_id)
    bio = io.BytesIO()
    await file.download_to_memory(bio)
    image_bytes = bio.getvalue()
    extracted = model_provider.image_to_text(
        image_bytes,
        mime_type=effective_mime or "image/jpeg",
    )
    return combine_caption_and_extracted(
        getattr(message, "caption", "") or "", extracted
    )


def message_sender_name(message: Any) -> str:
    from_user = getattr(message, "from_user", None)
    if from_user is None:
        return "Unknown"
    sender = getattr(from_user, "first_name", None) or getattr(from_user, "name", None)
    if sender:
        return str(sender)
    user_id = getattr(from_user, "id", None)
    if user_id is None:
        return "Unknown"
    return str(user_id)


async def extract_reply_target_text(  # pylint: disable=too-many-return-statements,too-many-branches
    reply_message: Any,
    context: ContextTypes.DEFAULT_TYPE,
    *,
    model_provider: ModelProvider,
    logger: logging.Logger,
) -> Tuple[str, str]:
    reply_text = (getattr(reply_message, "text", "") or "").strip()
    if reply_text:
        return reply_text, "message_text"

    if getattr(reply_message, "photo", None):
        try:
            extracted = await extract_text_from_photo_message(
                reply_message,
                context,
                model_provider=model_provider,
            )
            extracted_text = extracted.strip()
            if extracted_text:
                return extracted_text, "fallback_ocr"
        except Exception as exc:  # pylint: disable=broad-exception-caught
            logger.warning("Failed to OCR replied photo", extra={"error": str(exc)})
        caption = (getattr(reply_message, "caption", "") or "").strip()
        if caption:
            return caption, "message_caption"
        return NON_TEXT_REPLY_PLACEHOLDER, "non_text"

    if getattr(reply_message, "document", None):
        document = reply_message.document
        if document.file_size is not None and document.file_size > IMAGE_MAX_BYTES:
            caption = (getattr(reply_message, "caption", "") or "").strip()
            if caption:
                return caption, "message_caption"
            return NON_TEXT_REPLY_PLACEHOLDER, "image_too_large"
        try:
            extracted_doc = await extract_text_from_image_document(
                reply_message,
                context,
                model_provider=model_provider,
            )
            if extracted_doc is not None:
                extracted_text = extracted_doc.strip()
                if extracted_text:
                    return extracted_text, "fallback_ocr"
        except Exception as exc:  # pylint: disable=broad-exception-caught
            logger.warning(
                "Failed to OCR replied image document", extra={"error": str(exc)}
            )
        caption = (getattr(reply_message, "caption", "") or "").strip()
        if caption:
            return caption, "message_caption"
        return NON_TEXT_REPLY_PLACEHOLDER, "non_text"

    reply_caption = (getattr(reply_message, "caption", "") or "").strip()
    if reply_caption:
        return reply_caption, "message_caption"
    return NON_TEXT_REPLY_PLACEHOLDER, "non_text"


async def resolve_reply_target_context(
    message: Any,
    *,
    chat_id: str,
    context: ContextTypes.DEFAULT_TYPE,
    model_provider: ModelProvider,
    logger: logging.Logger,
) -> Optional[Dict[str, str]]:
    reply_message = getattr(message, "reply_to_message", None)
    if reply_message is None:
        return None

    sender = message_sender_name(reply_message)
    reply_message_id = getattr(reply_message, "message_id", None)
    if reply_message_id is not None:
        stored_message = get_message_by_telegram_message_id(
            chat_id=chat_id,
            telegram_message_id=reply_message_id,
        )
        if stored_message is not None:
            stored_text = str(stored_message.get("text", "")).strip()
            if stored_text:
                return {
                    "sender": str(stored_message.get("sender", sender) or sender),
                    "text": stored_text,
                    "source": "history_lookup",
                }

    resolved_text, source = await extract_reply_target_text(
        reply_message,
        context,
        model_provider=model_provider,
        logger=logger,
    )
    return {
        "sender": sender,
        "text": resolved_text or NON_TEXT_REPLY_PLACEHOLDER,
        "source": source,
    }


async def transcribe_voice_message(
    voice: Voice,
    context: ContextTypes.DEFAULT_TYPE,
    *,
    model_provider: ModelProvider,
    logger: logging.Logger,
) -> str:
    if voice is None:
        return ""
    with tempfile.NamedTemporaryFile(delete=False, suffix=".ogg") as temp_audio:
        temp_audio_path = temp_audio.name
    try:
        file = await context.bot.get_file(voice.file_id)
        await file.download_to_drive(temp_audio_path)
        return transcribe_audio(temp_audio_path, model_provider)
    finally:
        try:
            os.remove(temp_audio_path)
        except OSError:
            logger.debug(
                "Failed to remove temporary voice file",
                extra={"path": temp_audio_path},
            )
